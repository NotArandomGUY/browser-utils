...
export const getYTLocalEntityByType = async <T extends keyof YTLocalEntityData | void = void>(entityType: T extends void ? EntityType : T, decrypt: T extends void ? false : true): Promise<YTLocalEntity<T> | null> => {
  const entity = await getLocalDataDB(0).transaction('EntityStore', async trans => {
...
export const getYTLocalEntitiesByType = async <T extends keyof YTLocalEntityData | void = void>(entityType: T extends void ? EntityType : T, decrypt: T extends void ? false : true): Promise<YTLocalEntity<T>[]> => {
  return decodeYTLocalEntities(await getLocalDataDB(0).transaction('EntityStore', trans => {
    return trans.objectStore('EntityStore').index('entityType').getAll<YTLocalEntity[]>(typeof entityType === 'string' ? entityType : EntityType[entityType])
  }), decrypt)
}
...
export const getYTLocalEntitiesByKey = async <T extends keyof YTLocalEntityData | void = void>(entityKey: string, decrypt: T extends void ? false : true): Promise<YTLocalEntity<T>[]> => {
  return decodeYTLocalEntities(await getLocalDataDB(0).transaction('EntityStore', trans => {
    return trans.objectStore('EntityStore').getAll<YTLocalEntity[]>(entityKey)
  }), decrypt)
}
...
export const getYTLocalEntities = async <D extends boolean>(decrypt: D): Promise<YTLocalEntity<D extends true ? keyof YTLocalEntityData : void>[]> => {
  return decodeYTLocalEntities(await getLocalDataDB(0).transaction('EntityStore', trans => {
    return trans.objectStore('EntityStore').getAll<YTLocalEntity[]>()
...
export const getYTLocalMediaCaptions = async (id: string): Promise<YTLocalMediaCaption[]> => {
  return getLocalDataDB(1).transaction('captions', trans => {
    return trans.objectStore('captions').getAll(IDBKeyRange.bound(`${id}|`, `${id}~`))
  })
}

export const getYTLocalMediaIndex = async <T extends YTLocalMediaType>(id: string, type: T): Promise<YTLocalMediaIndex<T>> => {
  const index = await getLocalDataDB(1).transaction('index', trans => {
    return trans.objectStore('index').get<YTLocalMediaIndex<T>>(`${id}|${['a', 'v'][type]}`)
  })
...
export const getYTLocalMediaChunks = (
  ...
  try {
    ...
    resolve(await getLocalDataDB(1).transaction('media', trans => {
...
export const putYTLocalEntityAssociation = async (associations: YTLocalEntityAssociation[]): Promise<void> => {
  await getLocalDataDB(0).transaction('EntityAssociationStore', trans => {
...
export const putYTLocalMediaCaptions = async (id: string, captions: YTLocalMediaCaption[]): Promise<void> => {
  await getLocalDataDB(1).transaction('captions', trans => {
...
export const putYTLocalMediaStream = (index: YTLocalMediaIndex, reader: YTLocalMediaChunkReader, transforms: YTLocalMediaChunkTransform[] = []) => new PromiseWithProgress<void, number>(async (resolve, reject, progress) => {
  ...
  try {
    ...
    // Decrypt chunk transform (if it was encrypted)
    let chunkKey = bufferFromString(params.get('ck')?.padEnd(16, '\x00') ?? '')
    let chunkIv = bufferFromString(params.get('civ')?.padEnd(16, '\x00') ?? '')
    if (chunkKey.length >= 16 && chunkIv.length >= 16) transforms.unshift(decryptAesCtr.bind(null, chunkKey, chunkIv))

    // Encrypt chunk transform
    chunkKey = bufferFromString(getNonce(16))
    chunkIv = bufferFromString(getNonce(16))
    transforms.push(encryptAesCtr.bind(null, chunkKey, chunkIv))
    ...
    // Write index
    await db.transaction('index', async trans => {
      const key = `${videoId}|${['a', 'v'][index.type]}`
      await trans.objectStore('index').put({ fmts: index.fmts, format: index.format }, key)
    ...
    await waitAllBatched(transferTasks, TRANSFER_BATCH_SIZE).progress(p => progress(p / chunkCount))

    resolve()
  } catch (error) {
    if (db == null || transactions.length === 0) return reject(error)

    db.transaction(['index', 'media'], trans => {
      const indexStore = trans.objectStore('index')
      const mediaStore = trans.objectStore('media')
      return Promise.all([indexStore.delete(transactions[0])].concat(transactions.slice(1).map(key => mediaStore.delete(key))))
    }).then(() => reject(error)).catch(error => reject(error))
  }
})
